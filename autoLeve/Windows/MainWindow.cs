        ...
        using (var child = ImRaii.Child("SomeChildWithAScrollbar", Vector2.Zero, true))
        {
            ...
            if (child.Success)
            {
                ...
                if (goatImage != null)
                {
                    ...
                    {
                        ImGui.Image(goatImage.ImGuiHandle, goatImage.Size);
                    }
                }
                else
                {
                ...
                var jobIconId = 62100 + localPlayer.ClassJob.RowId;
                var iconTexture = Plugin.TextureProvider.GetFromGameIcon(new GameIconLookup(jobIconId)).GetWrapOrEmpty();
                ImGui.Image(iconTexture.ImGuiHandle, new Vector2(28, 28) * ImGuiHelpers.GlobalScale);
                
                ImGui.SameLine();
                
                // If you want to see the Macro representation of this SeString use `.ToMacroString()`
                // More info about SeStrings: https://dalamud.dev/plugin-development/sestring/
                ImGui.Text(localPlayer.ClassJob.Value.Abbreviation.ToString());
                
                ImGui.SameLine();
                ImGui.Text($" [Level {localPlayer.Level}]");
